...
if 'chat_messages' not in st.session_state:
    st.session_state['chat_messages'] = [
        {"role": "assistant", "content": "안녕하세요! AI 이벤트 아키텍트입니다. 공간 배치 명령을 입력해주시면 도면에 즉시 반영해 드립니다. (예: '푸드존을 더 멀리 이동해줘', '화장실을 의료 센터 근처로 이동해줘')"}
    ]
...
def parse_and_apply_ai_command(prompt: str):
    ...
    if "멀리" in prompt_clean or "외곽" in prompt_clean or "분리" in prompt_clean:
        if target_key == "food":
            fac["x"] = 120
            fac["y"] = 450
            action_desc = "푸드존을 연기 및 인파 혼잡 방지를 위해 서쪽 최외곽 구역(X:120, Y:450)으로 이동했습니다."
        elif target_key == "toilet":
            fac["x"] = 880
            fac["y"] = 380
            action_desc = "화장실을 메인 무대와 거리를 둔 동쪽 외곽 구역(X:880, Y:380)으로 이동시켰습니다."
        else:
        ...
        if target_key == "toilet":
            ...
            action_desc = f"{fac['name']}을(를) 응급의료센터 인근 접근하기 쉬운 위치로 조정했습니다."
        changed = True
        ...
        action_desc = f"{fac['name']}을(를) 상단 구역(X:{fac['x']}, Y:{fac['y']})으로 이동했습니다."
        changed = True

    elif "아래" in prompt_clean or "남쪽" in prompt_clean or "하단" in prompt_clean:
        fac["y"] = min(620, fac["y"] + 120)
        action_desc = f"{fac['name']}을(를) 하단 구역(X:{fac['x']}, Y:{fac['y']})으로 이동했습니다."
        changed = True
...
if st.session_state['page'] == 'home':
    ...
    with st.expander("📌 행사 기본 정보 및 예산 설정", expanded=not st.session_state['digital_twin_generated']):
        with st.form("event_input_form"):
            ...
            if btn_gen:
                ...
                st.session_state['facilities'] = DEFAULT_FACILITIES.copy()
                st.toast("AI가 장소 규격에 맞는 디지털 트윈 도면을 생성했습니다!")
    ...
    if not st.session_state['digital_twin_generated']:
        ...
        with col_main_left:
            ...
            st.caption("📐 격자(Grid) 규격과 벽체 치수가 측정되는 스마트 2D 건축 디지털 트윈 도면입니다.")
            ...
            st.markdown("**👇 시설 이름을 누르면 오른쪽에 배치 사유가 안내되며 직접 위치 조율이 가능합니다:**")
            ...
            with st.expander(f"⚙️ 선택된 [{cur_fac['name']}] 정밀 좌표 직접 조율하기", expanded=False):
        ...
        with col_main_right:
            ...
            if st.session_state['simulated']:
                st.caption("공간별 밀집도 예측 결과 (붉은색: 인파 정체 위험 구역)")
                np.random.seed(42)
        ...
        <div class="custom-card">
            ...
            <table style="width:100%; border-collapse: collapse; margin-top: 10px;">
                ...
                <tr>
                    ...
                    <td style="padding: 10px; border-bottom: 1px solid #E2E8F0;">응급의료센터 및 출입구가 최단 직선 코스로 연결됨</td>
                </tr>
                <tr>
                    ...
                    <td style="padding: 10px; border-bottom: 1px solid #E2E8F0;"><span class="badge badge-excellent">매우 우수</span></td>
                    <td style="padding: 10px; border-bottom: 1px solid #E2E8F0;">푸드존({food_pos})과 공연장({stage_pos}) 간 연기 간섭 최적 차단</td>
                </tr>
                <tr>
                    <td style="padding: 10px;">이용 편의성</td>
                    <td style="padding: 10px;">88점</td>
                    <td style="padding: 10px;"><span class="badge badge-good">우수</span></td>
                    <td style="padding: 10px;">화장실({toilet_pos}) 및 휴게 공간 동선 균형 확보</td>
                </tr>
            </table>
    ...
    with st.container():
        ...
        st.toggle("AI 챗봇 변경 시 자동 화면 갱신", value=True)
