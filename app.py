...
with st.sidebar:
    if st.button("🏗️ 이벤트 아키텍트 AI", key="logo_btn", use_container_width=True, type="primary"):
        st.session_state['page'] = 'home'
        st.rerun()
...
if st.session_state['page'] == 'home':
    ...
    st.caption("실제 축제장 현장 조감도 기반 입체 디지털 트윈 모드입니다.")
    ...
    if not st.session_state['digital_twin_generated']:
        ...
        with col_main_left:
            st.markdown("### 🗺️ 행사장 배치도 (디지털 트윈 기반)")
    ...
    # HTML 평가지표 및 수정한 보고서 컨텐츠
    st.markdown(f"""
        <div class="custom-card">
            <h2 style="color: #0F172A; text-align: center;">[보고서] 2026 청춘 페스티벌 공간 최적화 및 안전 설계안</h2>
            <p style="text-align: center; color: #64748B;">작성일: 2026년 9월 9일 | 시스템: 이벤트 아키텍트 AI | 보고 대상: 행사 총괄 책임자</p>
            <hr>
            ...
            <table style="width:100%; border-collapse: collapse; margin-top: 10px; font-size:0.95rem;">
                <tr style="background-color: #F1F5F9; text-align: left;">
                    <th style="padding: 10px; border-bottom: 2px solid #CBD5E1;">평가 항목</th>
                    <th style="padding: 10px; border-bottom: 2px solid #CBD5E1;">점수</th>
                    <th style="padding: 10px; border-bottom: 2px solid #CBD5E1;">등급</th>
                    <th style="padding: 10px; border-bottom: 2px solid #CBD5E1;">최신 배치 현황 및 평가 요약</th>
                </tr>
                <tr>
                    <td style="padding: 10px; border-bottom: 1px solid #E2E8F0;">안전성 및 비상 대피</td>
                    <td style="padding: 10px; border-bottom: 1px solid #E2E8F0; font-weight: bold; color:#15803D;">95점</td>
                ...
                <tr>
                    ...
                    <td style="padding: 10px; border-bottom: 1px solid #E2E8F0; font-weight: bold; color:#15803D;">90점</td>
                    <td style="padding: 10px; border-bottom: 1px solid #E2E8F0;"><span class="badge badge-excellent">매우 우수</span></td>
                    <td style="padding: 10px; border-bottom: 1px solid #E2E8F0;">푸드존과 메인 공연장 간 연기 간섭 최적 차단</td>
                </tr>
                <tr>
                    <td style="padding: 10px; border-bottom: 1px solid #E2E8F0;">이용 편의성</td>
                    <td style="padding: 10px; border-bottom: 1px solid #E2E8F0; font-weight: bold; color:#1E40AF;">88점</td>
                    <td style="padding: 10px; border-bottom: 1px solid #E2E8F0;"><span class="badge badge-good">우수</span></td>
                    <td style="padding: 10px; border-bottom: 1px solid #E2E8F0;">휴게 공간 및 주요 시설물의 균등 배치로 관람객 편의 제공</td>
                </tr>
            </table>

            <h3 style="margin-top:24px;">3. 현재 배치 시설 목록 및 사유</h3>
        </div>
    """, unsafe_allow_html=True)
    ...
    with r_col1:
        if st.button("📥 보고서 PDF 다운로드 (가상)", type="primary", use_container_width=True):
            st.toast("보고서 PDF 파일 다운로드가 시작되었습니다.")
    with r_col2:
        if st.button("🖨️ 보고서 인쇄하기", use_container_width=True):
            st.toast("인쇄 미리보기 화면으로 연결됩니다.")

elif st.session_state['page'] == 'settings':
    st.markdown("## ⚙️ 플랫폼 설정")
    st.caption("AI 이벤트 아키텍트 시스템의 기본 설정을 관리합니다.")
    ...
    with sc1:
        ...
        st.text_input("소속 부서", value="행사기획1팀")
    with sc2:
        st.text_input("이메일", value="user@event-architect.ai")
        st.selectbox("권한", ["총괄 기획자", "일반 기획자", "뷰어"])
    ...
    st.slider("인파 안전 가중치 (안전 vs 밀집)", 0, 100, 85)
    st.slider("동선 최적화 감도", 0, 100, 70)
    st.toggle("AI 자동 공간 재배치 제안 받기", value=True)
